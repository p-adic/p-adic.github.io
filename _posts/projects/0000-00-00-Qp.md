...
書き込まれた翻訳を`c`とすると、`c`の冒頭の`RelativePath/cpp/Mathematics/Function/Computable/Library/cpp/a.hpp`を`RP/cpp/Mathematics/Function/Computable/Library/cpp/a.hpp`に書き換え、`c`の後に改行を入れてそれより後に`int main()`の定義を記述することで、`int main()`内で`C`で定義された演算子をC++の関数として使うことが出来ます。それを通常のC++のソースコードとしてコンパイルし、再度`-lqp`をリンクすることで生成される実行ファイルを実行することで、`int main()`内に現れる`C`で定義された演算子をC++の関数として実際に計算することができます。

また糖衣表現は演算子でないので直接C++の関数として使うことは出来ませんが、型`T`の糖衣表現`e`に対し`e`をそのまま返り値とする高階型`->T`の演算子を導入することで実質糖衣表現もC++の関数として使うことが出来ます。例えば以下のように実装することが出来ます。
