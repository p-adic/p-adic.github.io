...
1. 白紙のcppファイルを用意し、それを$$c$$と置く。$$c$$の存在するカレントディレクトリから$$r$$への相対パスからを`RP`と置く。例えば$$r$$がカレントディレクトリに存在する場合は`RP`が`.`であり、$$r$$がカレントディレクトリの親ディレクトリに存在する場合は`RP`が`..`であり、$$r$$がカレントディレクトリの子ディレクトリ`child`に存在する場合は`RP`が`./child`である。
1. $$c$$に`RP/cpp/Mathematics/Function/Computable/a.hpp`をインクルードし、適宜改行をする。
1. $$c$$の末尾に`int main(){ C; return 0; }`というテキストを追加し、保存する。
1. $$c$$を$$G$$で十分新しい標準ライブラリ（例えば`-std=gnu++1z`）指定の下でコンパイルし、以下の手順でライブラリをリンクすることで生成される実行ファイルを$$e$$と置く。
  1. OSがWindowsの場合、オブジェクトファイル群`RP/cpp/Error/BreakPoint/a.o RP/cpp/Error/FaultInCoding/a.o RP/cpp/Error/IllegalAccess/a.o RP/cpp/Error/IllegalCall/a.o RP/cpp/Error/IllegalImput/a.o RP/cpp/Error/MismatchType/a.o RP/cpp/Error/Position/a.o RP/cpp/Error/Warning/a.o RP/cpp/Error/a.o RP/cpp/Utility/String/a.o RP/cpp/Mathematics/Arithmetic/Prime/a.o RP/cpp/Mathematics/Function/Computable/Expression/Primitive/bool/a.o RP/cpp/Mathematics/Function/Computable/Expression/Primitive/int/a.o RP/cpp/Mathematics/Function/Computable/Expression/Primitive/string/a.o RP/cpp/Mathematics/Function/Computable/Operator/Basic/bool/a.o RP/cpp/Mathematics/Function/Computable/Syntax/Translation/a.o RP/cpp/Mathematics/Function/Computable/Syntax/a.o RP/cpp/Mathematics/Function/Computable/Type/a.o`をリンクする。
  1. OSがLINUXの場合、`-LRP -lqb`をリンクする。
