...
導入の章で述べた通常の手順において`Mathematics/Function/Computable/a.hpp`の代わりに`Mathematics/Function/Computable/Library/cpp/a.hpp`をインクルードし、コンマを含まない文字列`F`とオープンモード`M`に対し`CONVERT_CPP(F,M); int main(){C; return 0;}`というコードをコンパイルして実行すると、カレントディレクトリからの相対パスが`F`のファイルに`M`に対応するオープンモードで`C`のC++への翻訳を書き込みます。
