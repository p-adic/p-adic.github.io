...
1. $$c$$を$$G$$でコンパイルし、`-lqp`をリンクすることで生成される実行ファイルを$$e$$と置く。ただしコンパイルには十分新しいコンパイラを用い、コンパイル時に`-std=gnu++1z`などで十分新しい標準ライブラリを使えるようにし、また`-L`でライブラリ`libqp.a`と`libqp.lib`の置かれているフォルダの相対パスを指定する。
