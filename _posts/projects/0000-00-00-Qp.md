...
1. $$r$$の`Mathematics/Function/Computable/Library/libqp.a`と`Mathematics/Function/Computable/Library/libqp.lib`を適切なフォルダにコピーする。
...
ただしコンパイルには十分新しいコンパイラを用い、コンパイル時に`-std=gnu++1z`などで十分新しい標準ライブラリを使えるようにし、また`-L`でライブラリ`libqp.a`と`libqp.lib`の置かれているフォルダの相対パスを指定して下さい。`-lqp`のリンクがうまく行かない場合は`-lqp`を`libqp.a`または`libqp.lib`に相対パス付きで置き換えて下さい。
