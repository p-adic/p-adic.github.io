...
- 例えば単調増加／減少関数$f$を用いた等式条件$f(x) = K$を満たす整数$x$を求める問題において、代わりに$=$を$\leq$／$\geq$に変更した条件を満たす$x$の最小値／最大値を求める方法を考え、以下の考察に繋げる。
  - 求解問題を二分探索で解く。
...
非空有限集合$S$と関数$f \colon S \to \mathbb{R}$が与えられ、$\min f(S)$や$\max f(S)$を求める問題を考える。

集合$X,Y$と全単射$i \colon S \to X \times Y$と写像$g_1 \colon X \to \R$と$g_2 \colon Y \to \R$と辞書式順序に関する順序保存写像$h \colon \R^2 \to \R$を用いて$f = h \circ (g_1 \times g_2) \circ i$と表すことで、$\max f(S)$や$\min f(S)$の計算を$h(\max g_1(X),\max g_2(Y))$や$h(\min g_1(X),\min g_2(Y))$に帰着させる手法。

平たく言えば、最大・最小化したい関数の値を独立な項に分解してそれぞれを最大・最小化することで全探索などの計算量を下げるテクニックである。
