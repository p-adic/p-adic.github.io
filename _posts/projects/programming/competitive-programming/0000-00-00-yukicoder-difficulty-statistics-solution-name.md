...
1. $G$を更新する処理をシミュレーションする。
    ...
    1. 連結成分$C_1$と$C_2$が結合されて連結成分$C_{1,2}$となる場合、$F[r(C_{1,2})]$を$F[r(C_1)] \otimes F[r(C_2)]$に置き換えることで$F$を更新する。

また$G$の連結成分$C$をわたる$f(C)$全体を用いて定義される値$g(G)$を求める必要がある場合、$g(G)$の値を格納する変数$x$を追加で用意し、$F$に並行して更新していく。

もし$F$に並行して$x$の更新が高速に行えない場合は、$F$のデータに[低次項などを追加して高速化]({{ site.url }}/yukicoder-difficulty-statistics#低次項の追加による線形化)を行う。

1. 例えば$G$の各頂点$v$に整数$A[v]$が定義されており$f(C)$が$C$の各頂点$v$をわたる$A[v]$の総和である場合は、$\otimes$は整数の加法である。
    1. [動的なグラフの連結成分の頂点数取得]({{ site.url }}/yukicoder-difficulty-statistics#動的なグラフの連結成分の頂点数取得)を要求する問題、すなわち$f(C)$を$C$の頂点数$V(C)$とする問題は、$A[v] = 1$の場合に相当する。
    1. $A$の一点加算クエリは$F$の一点加算クエリに翻訳可能である。実際、頂点$v$と整数$c$がクエリで与えられ$A[v]$に$c$を加算する際は、$v$の連結成分を$C$として$F[r(C)]$に$c$を加算すればよい。
1. 例えば$G$の各頂点$v$に整数$A[v]$が定義されており$f(C)$が$C$の各頂点$v$をわたる$A[v]$の総乗である場合は、$\otimes$は整数の乗法である。
    1. $A$の一点乗算クエリは$F$の一点乗算クエリに翻訳可能である。実際、頂点$v$と整数$c$がクエリで与えられ$A[v]$に$c$を乗算する際は、$v$の連結成分を$C$として$F[r(C)]$に$c$を乗算すればよい。
1. 例えば$f(C)$を$C$の頂点数の$2$乗$V(C)^2$とする問題は、$\otimes$を形式的には「平方根を取ってから足して$2$乗する」という演算にすればよいが、実装上は$f(C)$を低次項$V(C)$と定義し直し1-1に帰着させれば良いので平方根が不要である。
    1. $V(C)^2$が必要な時は$f(C)^2$を参照すれば良い。
    1. $g(G)$が$G$の連結成分$C$をわたる$V(C)^2$の総和として定義され$g(G)$を求める必要がある設定であれば、連結成分$C_1$と$C_2$が結合されて連結成分$C_{1,2}$となる場合、$x$に$2V(C_1)V(C_2) = 2f(C_1)f(C_2)$を加算すれば良い。
1. 例えば$f(C)$が$C$の頂点全体の集合の場合は、$\otimes$は集合の排他的和集合である。帰属判定は基点の比較に帰着できるので、実装上はsetを使わずともvectorを使えば良い。
1. 例えば$f(C)$が$C$の頂点の$1$つならば何でも良い場合は、$f(C) = r(C)$として$\otimes$は結合後の連結成分の基点を返す必ずしも可換でない$2$項演算とすれば良い。
    1. $g(G)$が$G$の連結成分$C$をわたる$f(C)$全体の集合と定義され$g(G)$を求める必要がある設定であれば、連結成分$C_1$と$C_2$が結合されて連結成分$C_{1,2}$となる場合、$x$から$r(C_1) = f(C_1)$と$r(C_2) = f(C_2)$を削除して$r(C_{1,2}) = f(C_{1,2})$を追加すれば良い。
...
　
{% assign solution = "単調関数の緩和計算" %}
<h2 id="{{ solution }}"><a href="{{ site.url }}/yukicoder-difficulty-statistics#{{ solution }}">{{ solution }}</a></h2>

<a href="#緩和">緩和</a>の具体例。全順序集合$I$と単調関数$f \colon I \to \mathbb{R}$が与えられているとする。

- 与えられた$Y \in \mathbb{R}$に対し$f(i) = Y$を満たす$i \in I$の存在判定や存在する時の最大・最小値を求める問題
- $I$を有限として各$i \in I$に対する$f(i)$を計算する問題

などを考える。

前者の問題を$f(i) \leq Y$や$f(i) \geq Y$に緩和し、その条件を満たす$i \in I$の最大・最小値を二分探索や式変形で求める。その$i$に対し$f(i) = Y$が成り立つか否かを判定することで、$f(i) = Y$を満たす$i \in I$の存在判定と存在する時の最大・最小値計算をする手法。

その応用として後者の問題も、$I$の部分集合$J$であって$I \setminus J$と$f_J$の像$f(J)$がともに小さいものが取れれば各$i \in I \setminu J$に対しては$f(i)$を愚直に求め、各$y \in f(J)$に対して$f(i) \leq y$を満たす最大の$i \in J$を求め$g(y)$と置くことで、$g$に対する二分探索に帰着できる。

ただし追加で

- 任意の$i \in I$に対し、$f(i)$を含む区間$J$が与えられた時$f(i)$の計算量が$\# J$の線形オーダーで抑えられる
- $I = [1,N] \cap \mathbb{Z}$かつ任意の$i \in I$に対し$f(i) \in [0,N/i] \cap \mathbb{Z}$が成り立つ（cf. https://codeforces.com/blog/entry/134445）

のいずれかが成り立てば、monotone minimaのような$I$を再帰的に二等分する手法の方が高速になりうることに注意。
