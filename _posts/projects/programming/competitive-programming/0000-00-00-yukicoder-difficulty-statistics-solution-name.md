...
　
<h2 id="指定序数の値の計算を分割の先頭項管理で処理">指定序数の値の計算を分割の先頭項管理で処理</h2>

全順序集合$S$とその要素数$N$以下の正整数$K$が与えられ、$S$の$K$番目の要素を求める問題を考える。

$M$を$N$より十分小さい正整数とし、$S$が$M$個の非空部分集合の族$(S_j)_{j=1}^{M}$に排他的に分割され、$M$以下の各正整数$j$に対し後続関数$S_j \setminus \\{\max S_j\\} \to S_j \setminus \\{\min S_j\\}$が高速に計算できるとする。
