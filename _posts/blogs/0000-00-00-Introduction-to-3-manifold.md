...
{% assign i = i | plus: 1 %}
### {{i}}：曖昧
単体的複体$$K$$に対しその幾何的実現のような「多面体」$$\lvert K \rvert$$という概念が定義されました。しかし$$\mathbb{R}^n$$を$$\mathbb{R}^{n+1}$$の部分空間とみなしている流儀なのかどうかがはっきり明言されていないことに注意します。またこの時点で多面体を図形とみなしているかどうかが不明確であるため、多面体がユークリッド空間に含まれるかどうかは両方の可能性を保持していきます。
