...
## コンパクト位相空間上の任意の実連続関数が最大値を持つことは選択公理なしで証明できますか？

いいえ、$\textsf{ZF}$集合論が無矛盾であるならば、コンパクト位相空間上の任意の実連続関数が最大値を持つことは$\textsf{ZF}$集合論で証明できません。
...
## 任意の可換環が極大イデアルを持つことは選択公理と同値ですか？

いいえ、$\textsf{ZF}$集合論が無矛盾であるならば、$\textsf{ZF}$集合論において任意の可換環が極大イデアルを持つことは選択公理と同値ではありません。
...
## 無理数の無理数乗が有理数になり得ることは背理法なしで証明できますか？

はい、背理法の代わりに排中律などを用いて証明することができます。

まず$\sqrt{2}$が無理数であることが$\neg$導入によって証明できます。

次に$(\sqrt{2})^{\sqrt{2}}$が有理数であるかまたは無理数であるということが排中律によって証明できます。

前者の仮定のもとで、無理数$\sqrt{2}$の無理数$\sqrt{2}$乗$(\sqrt{2})^{\sqrt{2}}$が有理数になリます。

後者の仮定のもとで、無理数$(\sqrt{2})^{\sqrt{2}}$の無理数$\sqrt{2}$乗$((\sqrt{2})^{\sqrt{2}})^{\sqrt{2}} = (\sqrt{2})^2 = 2$が有理数になリます。

従って$\lor$除去により無理数の無理数乗が有理数になり得るということが証明できました。
...
## $\zeta$関数の任意の非自明零点は実部が$\frac{1}{2}$の複素数ですか？
...
「アーベル圏と言ったら小圏を指すに決まってるじゃないですか」と後出しで条件を追加する反論が有効です。ただしその場合は一生$\textrm{Ab}$がアーベル圏でないという流儀に立つ覚悟が必要です。
