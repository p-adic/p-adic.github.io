...
この記事で集合やクラスを形式的に扱う際は、断りのない限り$\mathsf{ZFC}$公理系か$\textsf{NBG}$公理系を暗黙に課し（どちらでも通用する議論を行い）、諸概念がその中で形式化されているか否かを考えます。双方の公理系におけるクラスの扱いは形式的な圏論に置いて前提知識とされるので、ここでも既知のものとします。
