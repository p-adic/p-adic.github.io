...
なお「区間から小圏への関手が乗る」という表現の利点の１つは、端点情報付きの半開区間列$([0,1),[1,2),\ldots,[N-1,N))$がセグメント木に乗ることが即座に分かることです。区間$[0,N)$の射の列$((0,1),(1,2),\ldots,(N-1,N))$に翻訳すれば恒等関手を考えるだけですからね。ここで「端点情報付きの」と言っているのは、集合としては単なる空集合である半開区間$[0,0)$と$[1,1)$などを区別するためです。道の小圏を考える時に空の道にも始端と終端の情報があったことと同じです。
