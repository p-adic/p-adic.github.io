...
例えばセグメント木に区間から線形写像の小圏への関手を乗せる場合、恒等射を保つという条件が外れると作用によって各ベクトル空間の次元すら変わりうる（コンパイル時に固定できない）ものすら許容されるということです。

しかしやはりこの十分条件も区間代入や区間加算をカバーしていません。そこで、もう少し実用上都合の良い十分条件を与えましょう。

### 基点つきマグマから区間から小圏への作用モノイドへの準同型

ここからは圏論にある程度詳しい人向けの説明として、圏論の記法と用語を断りなく用います。まず小圏$C$と$D$が与えられているとします。

ここだけの用語で自己写像$F \in \textrm{End}(\textrm{Hom}_D)$が$D$の自己変換であることを以下の2条件を満たすこととして定義します。

1. 任意の$v \in \textrm{Ob}(D)$に対し、$g(\textrm{id}_v) = \textrm{id}_v$である。
1. 任意の$(v_0,v_1) \in \textrm{Ob}(D)^2$と$m \in \textrm{Hom}_D(v_0,v_1)$に対し、$g(m) \in \textrm{Hom}_D(v_0,v_1)$である。

これは対象を保つ自己関手にも似ていますが、関手性（合成の整合性）は課されていないことに注意しましょう。

$D$の自己全体の集合を$\textrm{Trans}(D)$と置きます。$\textrm{Trans}(D)$は$\textrm{End}(\textrm{Hom}_D)$の部分モノイドとなります。

ここだけの用語で$C$から$D$への作用とは、写像$F \colon \textrm{Hom}_C \to \textrm{Trans}(D)$の組であって以下を満たすもののこととします。

- 任意の$(\ell_0,\ell_1) \in \textrm{Hom}_C^{[2]}$と$(m_0,m_1) \in \textrm{Hom}_D^{[2]}$に対し、$g(\ell_0)(m_0) \circ g(\ell_1)(m_1) = g(\ell_0 \circ \ell_1)(m_0 \circ m_1)$である。

$C$から$D$の自己関手圏への関手とは合成に課される方向が違う（関手の合成方向ではなく射の合成方向を見る）ことに注意してください。

$C$から$D$への作用全体の集合を$\textrm{Act}(C,D)$と置きます。$\textrm{Act}(C,D)$は直積モノイド$\textrm{Trans}(D)^{\textrm{Hom}_C}$の部分モノイドとなります。

このように小圏から小圏への作用のモノイドを定式化すると、セグメント木の範囲更新が遅延評価できるための十分条件が得られます。

セグメント木に区間$[0,N)$から小圏$D$への関手を乗せているとします。例えば「基点つきマグマから$\textrm{Act}([0,N),D)$への準同型であって基点を単位元に写すものが与えられた時」、それによる範囲更新は遅延評価が可能です。
