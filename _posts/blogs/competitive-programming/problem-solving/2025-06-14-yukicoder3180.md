...
複素数$\alpha,\beta,\gamma$をそれぞれ$A_X + A_Y i,B_X + B_Y i,C_X + C_Y i$と置くと、度もワブルの法則より
...
これは実部と虚部をそれらの最大公約数で割って得られる複素数の一致で判定すれば良いです。つまりは$\alpha \beta$を$\delta$と置いて

\\[
\frac{\delta}{\gcd(\Re(\delta),\Im(\delta))} = \frac{\gamma}{\textrm{gcd}(\gamma)}
\\]
