...
つまり、離散フーリエ変換はゲルファント変換の特別なものということです。そして離散フーリエ変換は後で説明する巡回畳み込みの場合が特に分かりやすいです。
...
巡回群の指標や加法群$\mathbb{F}_2$の直和の指標の場合は、基底との内積による座標変換が再帰なりによって高速化できました。高速化の背景には指標のどんな性質が使われているでしょう？
...
裏を返せば$G$が巡回群であっても位数が$2 \times 3 \times 5 \times \cdots$のように無平方であれば高速フーリエ変換と同様の再帰が深度$1$で止まってしまいますし、$G$が加法群$\mathbb{F}_2$の直和群であっても次元が$2 \times 3 \times 5 \times \cdots$のように無平方であれば高速アダマール変換と同様の再帰が深度$1$で止まってしまいます。

ポントリャーギン双対の生成系に、再帰と相性の良い構造を見つけられるか否かという問題なわけですね。
...
さて巡回畳み込みについて説明した際に触れたように、特に$p-1$が$2$でいっぱい割れる時、$D$が$2$冪の場合の基底が取れて離散フーリエ変換ができ、その高速化が高速フーリエ変換（FFT）でした。従って$N$次未満の多項式の乗算が$D$次巡回畳み込みのFFTを用いて高速化が可能というわけです。多項式乗算やその$N$次打ち切りそのものがFFTなわけではないので、用語の混乱をしないように気をつけていきましょう。
