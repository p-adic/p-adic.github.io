...
畳み込み積に関して$v \in K^N$との内積$K^N \to K$が環準同型である必要十分条件は、$v$が$G$の指標であること、すなわち合成写像$G \hookrightarrow K[G] \cong K^N \to K$が$G$から$K$の乗法モノイドへのモノイド準同型であることです。指標についてよく知らない人はあまり気にせず次に説明する巡回畳み込みとbitwise XOR畳み込みにおける指標の具体例まで飛んでください。逆に言うと、具体例までは指標についての知識を仮定します。
...
多項式乗算の巡回畳み込みへの帰着によるゲルファント変換の活用をマスターしたら、上の問題にぜひ挑戦してみてください。
