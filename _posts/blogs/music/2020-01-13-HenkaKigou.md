...
\end{align}
\\]
通常の流儀では空文字列を変化記号と呼ぶことはしないと思いますが、今回は実装の便宜上空文字列を変化記号とみなしています。また$$\textrm{𝄫}$$（環境によっては正しく表示できません）や$$\textrm{♮}$$や$$\textrm{𝄪}$$（環境によっては正しく表示できません）等を変化記号として用いる流儀もありますが、ここではそれらを用いません。
