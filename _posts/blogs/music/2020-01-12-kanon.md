...
特にこの流儀では空列も文字列となりますが、今回は長さ$$2$$の文字列しか扱っていないため空列の扱いを気にする必要はありません。７種類の幹音をその順に$$0,1,2,3,4,5,6 \in \mathbb{Z}/7 \mathbb{Z}$$に対応させることで、$$\textrm{KanOn}$$に環構造を誘導します。
...
- inline const Mod<7>& operator-( const KanOn& N1 , const KanOn& N2 ) noexcept`はN1.GetNum() - N2.GetNum()`で定める。
