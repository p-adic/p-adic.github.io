...
- `KanOn& KanOn::operator++( int ) noexcept`と`KanOn& KanOn::operator--( int ) noexcept`はそれぞれ`m_num += 1`と`m_num -= 1`を実行した上で`*this`への参照返しで定める。
- `inline const string& KanOn::Display() const noexcept`は`KanOn::IntToString( m_num )`で定める。
- `inline Mod<7> KanOn::GetNum() const noexcept`は`KanOn::m_num`への参照返しで定める。
