...
特にこの流儀では空列も文字列となりますが、今回は長さ$$2$$の文字列しか扱っていないため空列の扱いを気にする必要はありません。$$7$$種類の幹音を上の順に$$0,1,2,3,4,5,6 \in \mathbb{Z}/7 \mathbb{Z}$$に対応させることで、全単射な写像
\\[
\textrm{GetNum} \colon \textrm{KanOn} \to \mathbb{Z}/7 \mathbb{Z}
\\]
を得るので、これを通じて$$\textrm{KanOn}$$に環構造を誘導します。
