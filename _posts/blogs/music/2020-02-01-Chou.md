...
\\]
を以下のように定めます：
1. $$N = (N_0,S_0) \in \textrm{KanOn} \times \textrm{HenkaKigou}$$と置く。
...
- `inline const OnMei& Chou::GetOnMei() const noexcept`は`return m_N`で定める。
- `inline const OnKai& Chou::GetOnKai() const noexcep`は`return m_scale`で定める。
- `inline string Chou::Display() const noexcept`は`return m_scale.Display() + ": " + m_N.Display()`で定める。
- `inline KanOn Chou::KanOnTable( const KaiMei& num ) const noexcept`は`return m_N.GetKanOn() + num`で定める。
- `inline HenkaKigou Chou::HenkaKigouTable( const KaiMei& num ) const noexcept`は`return HenkaKigouTable_Body( num , KanOnTable( num ) )`で定める。
- `inline OnMei Chou::OnMeiTable( const KaiMei& num ) const noexcept`は`return OnMeiTable_Body( num , KanOnTable( num ))`で定める。
- `HenkaKigou Chou::HenkaKigouTable_Body( const KaiMei& num , const KanOn& N ) const noexcept;`は`const int d`を`( OnMei::KanOnToPitchClass( m_N.GetKanOn() ) + m_scale.PitchClassTable( num ) - OnMei( N ).GetPitchClass() ).Represent()`の戻り値で定め、`const HenkaKigou& S`を`m_N.GetHenkaKigou()`の戻り値を返し、`d < 6`ならば`return HenkaKigou( d ) + S`で定め、そうでないならば`return HenkaKigou( d - 12 ) + S`を返す。
- `inline OnMei Chou::OnMeiTable_Body( const KaiMei& num , const KanOn& N ) const noexcept`は`return OnMei( N , HenkaKigouTable_Body( num , N ) )`で定める。
