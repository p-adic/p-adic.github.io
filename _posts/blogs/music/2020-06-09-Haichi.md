...
$$P$$を配置とします。$$\textrm{GetPitch}(P,0)$$を***$$P$$のバス***、$$\textrm{GetPitch}(P,1)$$を***$$P$$のテナー***、$$\textrm{GetPitch}(P,2)$$を***$$P$$のアルト***、$$\textrm{GetPitch}(P,3)$$を***$$P$$のソプラノ***と呼びます。音名の集合$$\{\textrm{GetOnMei}(\textrm{GetPitch}(P,i)) \mid i \in \{0,1,2,3\}\}$$を***$$P$$の音名集合***と呼び、その要素数を$$\textrm{GetNumberOfOnMei}(P) \in \{1,2,3,4\}$$と置きます。$$\textrm{GetNumberOfOnMei}(P) = 3$$の時$$P$$を***$$3$和音の配置***と呼び、$$\textrm{GetNumberOfOnMei}(P) = 4$$の時$$P$$を***$$4$和音の配置***と呼びます。また$$3$$和音または$$4$$和音である同値類$$N$$が$$P$$の音名集合と同一視される時、$$P$$を***$$N$$の配置***と呼びます。
