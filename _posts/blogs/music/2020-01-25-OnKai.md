...
- `const OnKai& ChouOnKai() noexcept`はコンストラクタ`OnKai::OnKai( "" , 0 , 2 , 4 , 5 , 7 , 9 , 11 )`で構築される`const OnKai`型静的局所変数への参照返しで定める。
- `const OnKai& WaseiTekiTanOnKai() noexcept`はコンストラクタ`OnKai::OnKai( "m" , 0 , 2 , 3 , 5 , 7 , 8 , 11 )`で構築される`const OnKai`型静的局所変数への参照返しで定める。
