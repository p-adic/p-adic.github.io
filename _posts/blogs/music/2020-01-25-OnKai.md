...
$$\textrm{KaiMei} = \mathbb{Z}/7 \mathbb{Z}$$と置き、$$\textrm{KaiMei}$$の元を***階名***と呼びます。通常の流儀では$$\textrm{KaiMei} = \textrm{KanOn}$$なり$$\textrm{KaiMei} = \textrm{OnMei}$$なりと定義して$$\textrm{KaiMei}$$の元を文字列とすべきですが、ここでは実装の都合で階名を数のクラスとしてしまいます。また写像
\\[
\begin{align}
