...
- `inline KyouWaOn::KyouWaOn( const Chou& N , const KaiMei& n ) noexcept`はメンバ初期化子`SanWaOn( N.OnMeiTable( n ) , N.OnMeiTable( n + 2 ) , N.OnMeiTable( n + 4 ) )` , `m_is_valid( n != KaiMei::VII() )`で定める。
