...
調$$N$$と階名$$n$$に対し、
\\[
H_{N,n} = \{\textrm{OnMeiTable}(N,n),\textrm{OnMeiTable}(N,n+2),\textrm{OnMeiTable}(N,n+4)\}
\\]
と置きます。$$n,n+2,n+4 \in \textrm{KaiMei}$$が互いに異なることから$$H_{N,n}$$は濃度$$3$$の$$\textrm{OnMei}$$の部分集合となります。[$$3$$和音の記事]({{ site.url }}/SanWaOn)で導入した同一視により、$$H_{N,n}$$を$$3$$和音とみなします。
...
((N,n),i) & \mapsto \textrm{GetOnMei}(\textrm{OnMeiTable}(N,n),\textrm{OnMeiTable}(N,n+2),\textrm{OnMeiTable}(N,n*+4))
\end{align}
\\]
は全射
\\[
