...
$$n \inn \mathbb{N} \setminus \\{0\\}$$に対し、``Mod<n>``で$$\mathbb{Z}/n \mathbb{Z}$$に相当するクラスを表します。幹音のクラス`KanOn`および関係する関数たちを以下のように宣言します。
...
- `inline string& KanOn::Display() const noexcept`は`KanOnIntToString( m_num )`の値で定める。
