...
ただし$$n \in \mathbb{Z}$$の$$\mathbb{Z}/7 \mathbb{Z}$$での像$$n + 7 \mathbb{Z}$$もまた$$n$$と略記しています。一般に、環$$R$$に対して一意な環準同型$$\mathbb{Z} \to R$$による$$n \in \mathbb{Z}$$の像もまた曖昧さの致命的でない範囲で$$n$$と略記します。この写像$$\textrm{GetNum} \colon \textrm{KanOn} \to \mathbb{Z}/7 \mathbb{Z}$$は全単射であるため、これを通じて$$\textrm{KanOn}$$に環構造を誘導します。
