...
- `inline SanWaOn::SanWaOn( const OnMei& N0 , const OnMei& N1 , const OnMei& N2 ) noexcept`はメンバ初期化子`m_N0( N0 )` , `m_N1( N1 )` , `m_N2( N2 )` , ``m_is_valid( m_N0 != m_N1 && m_N1 != m_N2 && m_N2 != m_N0 )`で定める。
...
- `inline const bool& SanWaOn::IsValid() const noexcept`は`return m_is_valid`定める。
