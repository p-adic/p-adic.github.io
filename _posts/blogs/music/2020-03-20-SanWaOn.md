...
\end{align}
\\]
を$$\textrm{GetOnMei}$$と置きます。すると$$\textrm{GetOnMei}$$はwell-definedな写像$$\overline{\textrm{SanWaOn}} \times \{0,1,2\} \to \textrm{OnMei}$$を誘導しません。実際、$$(\textrm{Do},\textrm{Re},\textrm{Mi}) \sim (\textrm{Re},\textrm{Do},\textrm{Mi})$$を満たす一方で$$\textrm{GetOnMei}((\textrm{Do},\textrm{Re},\textrm{Mi}),0) \textrm{Do} \neq \textrm{Re} = \textrm{GetOnMei}((\textrm{Re},\textrm{Do},\textrm{Mi}),0)$$となります。その意味で「$$3$$和音の$$1$$番目の音名」はill-definedな概念となります。代わりに後に導入する「$$3$$和音の配置の$$1$$番目のピッチ」等はwell-definedとなります。
