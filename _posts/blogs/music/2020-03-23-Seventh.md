...
調$$N$$と階名$$n$$に対し、
\\[
\textrm{Seventh}_{N,n} = (\textrm{OnMeiTable}(N,n),\textrm{OnMeiTable}(N,n+2),\textrm{OnMeiTable}(N,n+4),\textrm{OnMeiTable}(N,n+6))
\\]
...
以上により、$$\textrm{GetOnMei} \colon \textrm{YonWaOn} \times \{0,1,2,3\} \to \textrm{OnMei}$$が標準射影を通じて$$\overline{\textrm{YonWaOn}} \times \{0,1,2,3\}$$を経由しないという意味で「$$4$$和音の$$1$$番目の音名」等はill-definedであった一方で、七の和音のみに絞ることで「$$1$$番目の音名」等の代わりとなる概念として根音等の概念を自然に定義することができました。
